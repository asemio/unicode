...
let valid_encodings = ["UTF-8"; "UTF-16"; "UTF-16LE"; "UTF-16BE"; "ANSI_X3.4-1968"; "ISO-IR-6"; "ANSI_X3.4-1986"; "ISO_646.IRV:1991"; "ASCII"; "ISO646-US"; "US-ASCII"; "US"; "IBM367"; "CP367"; "CSASCII"; "ISO_8859-1:1987"; "ISO-IR-100"; "ISO_8859-1"; "ISO-8859-1"; "LATIN1"; "L1"; "IBM819"; "CP819"; "CSISOLATIN1"]
...
let recode ?nln ?encoding out_encoding
    (src : [`Channel of In_channel.t | `String of string])
    (dst : [`Channel of Out_channel.t | `Buffer of Buffer.t])
  =
  let rec loop d e = match Uutf.decode d with
  | `Uchar _ as u -> let _ : enc = Uutf.encode e u in loop d e
  | `End -> let _ : enc = Uutf.encode e `End in ()
  | `Malformed _ -> let _ : enc = Uutf.encode e (`Uchar Uutf.u_rep) in loop d e
  | `Await -> assert false
  in
...
let create ?encoding_name raw =
  let value = begin match encoding_name with
  ...
  | Some name ->
    ...
    | _ ->
      begin match Uutf.encoding_of_string name with
      | None -> Passthrough raw
...
let convert_1252 str =
  try Utf8conv.utf8_of_windows1252 str
  with _ -> failwithf "Not a valid Windows-1252/CP-1252 string, are you sure it's the right format? : %s" str ()
...
let raw_bytes box =
  begin match box.value with
  | Raw { raw; _ } -> raw
  | Passthrough bytes
  | Utf8 bytes
  | Utf8_len { bytes; _ }
  | Normalized bytes
  | Normalized_len { bytes; _ } -> bytes
  end

let utf8_bytes box =
  begin match box.value with
  | Raw { raw; encoding } ->
    let utf8 = begin match encoding with
...
let rebox ?length box bytes =
  let value = begin match box.value, length with
  | (Normalized _), (Some length)
  | (Normalized_len _), (Some length) -> Normalized_len { bytes; length }

  | (Normalized _), None
  | (Normalized_len _), None          -> Normalized bytes

  | (Raw _), (Some length)
  | (Passthrough _), (Some length)
  | (Utf8 _), (Some length)
  | (Utf8_len _), (Some length)       -> Utf8_len { bytes; length }

  | (Raw _), None
  | (Passthrough _), None
  | (Utf8 _), None
  | (Utf8_len _), None                -> Utf8 bytes
  end
  in
  { value }
...
let to_title box =
  ...
  let _ : bool = Uutf.String.fold_utf_8 (fun acc _ -> function
    | `Malformed _ ->
      Uutf.Buffer.add_utf_8 buf Uutf.u_rep;
      false
    | `Uchar u ->
      begin match (Uucp.White.is_white_space u), acc with
      | true, _ ->
        Uutf.Buffer.add_utf_8 buf u; true
      | false, true ->
        begin match Uucp.Case.Map.to_upper u with
        | `Self -> Uutf.Buffer.add_utf_8 buf u; false
        | `Uchars us -> List.iter us ~f:(Uutf.Buffer.add_utf_8 buf); false
        end
      | false, false ->
...
let length box =
  ...
  | Utf8_len { length; _ }
  | Normalized_len { length; _ } -> length
  end
...
let unaccent box =
  ...
  Uuseg_string.fold_utf_8 `Grapheme_cluster (fun () glyph ->
    begin match String.Map.find Unaccent_rules.lookup glyph with
...
let standardize ?(rep=' ') box =
  let bytes = cmp_bytes box in
  let buf = Buffer.create ((String.length bytes) + 10) in
  let _ : bool = Uuseg_string.fold_utf_8 `Grapheme_cluster (fun top_prev_junk glyph ->
      let chunk = begin match String.Map.find Unaccent_rules.lookup glyph with
      | Some rep -> rep
      | None -> glyph
      end
      in
...
let unicode_trim box =
  let bytes = utf8_bytes box in
  let left, right = Uutf.String.fold_utf_8 (fun acc i m ->
      begin match m with
      | `Malformed _ -> acc
      | `Uchar u ->
  ...
  in
  let sliced = String.slice bytes
      (Option.value ~default:0 left)
...
let squish box =
  ...
  let _, _, length = Uuseg_string.fold_utf_8 `Grapheme_cluster (fun acc glyph ->
      Uutf.String.fold_utf_8 (fun (after_first, prev_ws, len) i m ->
        begin match m with
...
let index box =
  ...
  let _ : int = Uuseg_string.fold_utf_8 `Grapheme_cluster (fun pos m ->
  ...
  bytes, {
    indexes = Queue.to_array indexes;
    lengths = Queue.to_array lengths;
    length = Queue.length indexes;
  }

let slice box u_from u_to =
  let bytes, lookup = index box in
  let result = if Int.(u_from >= lookup.length) || Int.((lookup.length - u_from + u_to) <= 0)
    then ""
    else
    ...
    let s_from = try Array.get lookup.indexes norm_from with _ -> 0 in
    let s_to = try (Array.get lookup.indexes norm_to) + (Array.get lookup.lengths norm_to) with _ -> ((Array.last lookup.indexes) + (Array.last lookup.lengths)) in
    if s_from > s_to then "" else
    String.slice bytes s_from s_to
  in
  rebox box result
