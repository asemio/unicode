...
let fold_neighbor ~gap ll =
  let rec loop = function
    | None, [] -> Int.Map.empty
    | Some acc, [] -> acc
    | None, first :: rest -> if Map.is_empty first then first else loop (Some first, rest)
    | Some acc, first :: rest ->
      let positions =
        Map.filter_mapi acc ~f:(fun ~key:position ~data:width ->
            Map.find first (position + width + gap - 1) |> Option.map ~f:(( + ) width))
      in
      if Map.is_empty positions then positions else loop (Some positions, rest)
  in
  loop (None, ll)

let union a b =
  Map.merge a b ~f:(fun ~key -> function
    | `Left x
     |`Right x ->
      Some x
...
| Token s -> (
  match Map.find btree s with
  | Some (`Indexed x) -> x
  | Some `Unindexed
   |None ->
    Int.Map.empty)
| Prefix prefix -> (
  Map.binary_search btree `Last_equal_to prefix ~compare:(fun ~key ~data:_ x ->
  ...
  | None -> Int.Map.empty
  | Some (last, _) ->
    Map.fold_range_inclusive btree ~min:prefix ~max:last ~init:Int.Map.empty
        ...
        | Indexed (key, _) ->
          let acc =
            Map.update map key ~f:(function
              ...
              | Some (`Indexed acc) -> `Indexed (Map.set acc ~key:i ~data:1))
          in
          i + 1, acc)
    in
...
let matches vector root_query =
  ...
  let rec loop : Tsquery.t -> bool = function
    | Token s -> Map.mem btree s
    | Prefix prefix ->
      Map.binary_search btree
    ...
    | Clause (NEIGHBOR _, _) as query -> get_positions btree query |> Map.is_empty |> not
  in
  loop root_query
